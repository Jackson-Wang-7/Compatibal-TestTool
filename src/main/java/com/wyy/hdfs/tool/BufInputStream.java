...
public class BufInputStream extends InputStream {
    ...
    private static Logger log = LoggerFactory.getLogger(BufInputStream.class);
    ...
    public BufInputStream(InputStream in, int bufSize) {
        this.in = in;
...
//                log.warn("[BufInputStream.fill] offset:{} buf.len:{} readSum:{} c:{}.",
//                        offSet, this.buf.length, readSum, c);
                if (-1 == c) {
                    this.finish = true;
                    break;
                } else if (0 != c) {
